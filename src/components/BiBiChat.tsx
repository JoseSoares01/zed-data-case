  ...
  return createPortal(
    <>
      ...
      >
        ...
        <div className="relative w-14 h-14 md:w-16 md:h-16 rounded-full bg-orange-500 flex items-end justify-center shadow-2xl transition-transform group-hover:scale-110 animate-[bibi-float_4s_ease-in-out_infinite] overflow-visible">
          <img
            src={EMOTIONS[open ? emotion : "tedio"]}
            alt="BiBi"
            className="w-[5.5rem] h-[5.5rem] md:w-24 md:h-24 object-contain drop-shadow-lg group-hover:-rotate-6 transition-transform absolute -top-7 md:-top-8"
          />
        </div>
      </button>
      ...
      {open && (
        <div className="fixed bottom-32 right-6 z-[9999] w-[calc(100vw-3rem)] max-w-sm h-[28rem] flex flex-col rounded-2xl shadow-2xl bg-neutral-900/95 backdrop-blur-xl border border-white/10 overflow-hidden animate-[bibi-in_0.25s_ease-out]">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-gradient-to-r from-orange-600/20 to-transparent">
            <div className="relative w-8 h-8 rounded-full bg-orange-500 flex items-end justify-center shrink-0 overflow-visible">
              <img src={EMOTIONS[emotion]} alt="" className="w-10 h-10 object-contain absolute -top-2.5" />
            </div>
